...
$(document).on('change', '.order-single-product .price-select', function () {
    ...
    let product = $(this).find('option:selected').data('product');
    let attribute = price.attributes.find(attribute => attribute.group.type === 'color');
    let attributePicture = product.pictures.find(picture => picture.attribute_id === attribute.id);

    $(this)
        .closest('.order-single-product')
        .find('.product-image')
        .attr('src', attributePicture.picture);
    $(this)
        .closest('.order-single-product')
        .find('.selected-price')
        .val(price.id);
