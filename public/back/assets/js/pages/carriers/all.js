...
$.each(provinces, function (index, province) {
    ...
    $.each(province.cities, function (index, city) {
        cities.push({
            id: city.id,
            title: city.name.toLowerCase().toPersianCharacter()
        });
    ...
    let p = {
        ...
        title: province.name.toLowerCase().toPersianCharacter(),
        subs: cities
    };
    ...
    $.ajax({
        ...
        success: function (data) {
            $(data).each(function (i, item) {
                $('#city').append(
                    '<option value="' + item.id + '">' + item.name.toLowerCase().toPersianCharacter() + '</option>'
                );
            });
        },
        beforeSend: function () {
