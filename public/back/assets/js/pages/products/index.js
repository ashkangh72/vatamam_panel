...
var product_datatable = function() {
    ...
    var options = {
        ...
        columns: [{
            ...
            },
            {
                ...
                template: function(row) {
                    ...
                    if (row.addableToCart) {
                        var addableToCartClass = 'badge-success';
                        var addableToCartText = 'موجود';
                    } else {
                        var addableToCartClass = 'badge-danger';
                        var addableToCartText = 'ناموجود';
                    }
                    return '<div class="badge badge-pill ' + addableToCartClass + ' badge-md">' + addableToCartText + '</div>';
                },
            },
            {
                field: 'published',
                ...
                template: function(row) {
                    ...
                    return '<div class="badge badge-pill ' + publishedClass + ' badge-md">' + publishedText + '</div>';
                },
            },
            {
                field: 'actions',
            ...
            },
            {
                ...
                width: 50,
                template: function(row) {
                    return '<a title="کپی کردن" href="' + row.links.copy + '" target="_blank"><i class="feather icon-copy"></i></a>\
                    <a title="مشاهده" href="' + row.links.front + '" target="_blank"><i class="feather icon-external-link"></i></a>';
                },
            }
        ],
    };
    ...
    var initDatatable = function() {
        ...
        $('#filter-products-btn').on('click', function(event) {
            event.preventDefault();
            formDataToUrl('filter-products-form');
            datatable.setDataSourceQuery($('#filter-products-form').serializeJSON());
            datatable.reload();
        });
