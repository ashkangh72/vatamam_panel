        ...
        serializeArray: function($form, opts) {
            ...
            return $form
                ...
                .filter(function() {
                    ...
                    return (
                        this.name && // must contain a name attribute
                        !$el.is(':disabled') && // must not be disable (use .is(":disabled") so that fieldset[disabled] works)
                        rsubmittable.test(this.nodeName) &&
                        !rsubmitterTypes.test(type) && // only serialize submittable fields (and not buttons)
                        (this.checked || !rcheckableType.test(type) || f.getCheckboxUncheckedValue($el, opts) != null)
                    ); // skip unchecked checkboxes (unless using opts)
                })
                .map(function(_i, el) {
                    var $el = $(this);
                    var val = $el.val();
...
(function ($) {
    $.fn.customPersianDate = function () {
        $(this).each(function (i, el) {
            if ($(this).data('customPersianDate')) {
                return;
            }
            ...
            $(this).data('customPersianDate', true);
        });
