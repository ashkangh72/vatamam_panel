...
let nestable = $('.dd').nestable({
    ...
    onDragStart: function (l, e) {
        let nochildren = ['.dd-static'];

        if ($(e).children().find('.dd-item').length > 0) {
            let $target = $(e).children().find('.dd-item');

            while ($target.length > 1) {
                $target = $target.children().find('.dd-item');
            }

            let selectedLenght = $($target[0]).parentsUntil($(e), '.dd-item').length + 1;
        } else {
...
let menus = $('.dd').nestable('serialize');
...
$(document).on('click', '.edit-menu', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#edit-form input[name="url"]').val(data.menu.url);
...
$('#edit-form').submit(function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            let item = $('a[data-menu=' + data.menu.id + ']').closest('.dd-item');
