    ...
    <form id="comment-edit-form" action="{{ route('admin.comments.update', ['comment' => $comment]) }}">
        ...
        <table class="table">
            <tbody>
                ...
                <tr>
                    <th scope="row" style="min-width: 100px;">عنوان دیدگاه</th>
                    <td>
                        <div id="comment-title">
                            {{ $comment->title }}

                            <div class="mt-1">
                                <button id="edit-title-btn" type="button" class="btn btn-flat-primary waves-effect waves-light"><i class="feather icon-edit"></i> ویرایش</button>
                            </div>
                        </div>


                        <fieldset id="edit-comment-title" class="form-group" style="display: none;">
                            <textarea name="title" class="form-control" rows="4" required>{{ $comment->title }}</textarea>
                        </fieldset>
                    </td>
