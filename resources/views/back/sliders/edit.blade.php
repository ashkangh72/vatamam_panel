                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-12 col-md-6">
                                            <fieldset class="form-group">
                                                ...
                                                <div class="custom-file">
                                                    <select name="link_type"  onchange="generateConnectHtml(event)"
                                                            id="connect" class="form-control">

                                                        <option value>انتخاب کنید</option>
                                                        <option value="product" {{$slider->link_type=='product'?'selected':''}} id="product" data-action="{{route('admin.products.search.title')}}">محصول</option>
                                                        <option value="category" {{$slider->link_type=='category'?'selected':''}} id="category" data-action="{{route('admin.categories.search.title')}}">دسته</option>
                                                    </select>
                                                </div>
                                            </fieldset>
                                        </div>
                                        <div class="col-12 col-md-6" id="connect-wrapper">
                                            <fieldset class="form-group">
                                                ...
                                                if ($slider->link_type=='product')
                                                    $linkable=\App\Models\Product::find($slider->link_id);
                                                elseif ($slider->link_type=='category')
                                                    $linkable=\App\Models\Category::find($slider->link_id);
                                                @endphp

                                                <select name="link_id" id="link_id" class="form-control">
                                                    @if($slider->link_type and $linkable)
                                                        <option selected value="{{$slider->link_id}}">{{$linkable->title}}</option>
                                                    @endif
                                                </select>
...
@push('scripts')
    ...
    <script src="{{ asset('public/back/assets/js/pages/sliders/edit.js') }}"></script>
@endpush
